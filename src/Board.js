...
function displaySquares(squares, setSelectedSquare) {
    ...
    for (let r = 0; r < squares.length; r++) {
        ...
        for (let c = 0; c < squares[r].length; c++) {
            row.push(<td><div> <Square setSelectedSquare={setSelectedSquare} letter={squares[r][c]} r={r} c={c} /></div></td>)
        }
        row.push(<th>{r}</th>);
...
export class Board extends Component {
    ...
    constructor(props) {
        super(props);
        this.state = {
            selectedSquare : {letter: "", row: -1, col: -1}
        }
    }

    setSelectedSquare = (event, letter, row, col) => {
        this.setState({selectedSquare : {letter, row, col}});
    } 
